...
use App\Main\CostoViajePrioritario;
...
class CostoViajePrioritarioTest extends TestCase
{
    public function test_calculo_de_costo_para_viaje_prioritario()
    {
        ...
        $viaje = new Viaje($origen, $destino, $paquetes, new CostoViajePrioritario());
    ...
    public function costoEsperado($origen, $destino, $paquetes)
    {
        ...
        $volumenTotal = array_sum(array_map(function ($paquete) {
            return $paquete->getAlto() * $paquete->getAncho() * $paquete->getLargo();
        }, $paquetes));

        return max(4 * $pesoTotal * $distancia, 10 * $volumenTotal * $distancia);
    }
}
